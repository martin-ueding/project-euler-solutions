...
def make_timing(callable) -> None:
    ...
    result = None
    while (
        datetime.datetime.now() - benchmark_start < datetime.timedelta(seconds=1)
        and len(timings_s) < 100
    ):
    ...
    print(
        f"Timings: {p0} | {p25} | {p50} | {p75} | {p100} | {len(timings_s)} iterations"
    )
...
def main() -> None:
    ...
    solution_x = importlib.import_module(
        f".solutions.solution_{problem_id}", "project_euler_python"
    )
